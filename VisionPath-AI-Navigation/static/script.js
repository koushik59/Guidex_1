...
if (SpeechRecognition && voiceBtn) {
    ...
    voiceBtn.addEventListener('click', () => {
        ...
        // Short delay to let TTS finish before microphone activates
        setTimeout(() => {
            try {
                recognition.start();
            } catch (e) {
    ...
    recognition.onresult = (event) => {
        ...
        if (command.includes('front') || command.includes('scene') || command.includes('describe')) {
            getSceneDescription();
        } else if (command.includes('read') || command.includes('written') || command.includes('text') || command.includes('paper') || command.includes('document')) {
            readTextFromCamera();
        } else if (command.includes('indoor')) {
            environmentModeSelect.value = 'indoor';
            handleEnvironmentChange();
        } else if (command.includes('outdoor')) {
            environmentModeSelect.value = 'outdoor';
            handleEnvironmentChange();
        } else if (command.includes('emergency') || command.includes('sos') || command.includes('call') || command.includes('help')) {
            activateSOS();
        } else {
            speak("Command not recognized. Try asking what's in front of me, or ask me to read text.");
        }
    };
